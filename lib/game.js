...
class Game{
	...
	start(){
		...
		document.getElementsByClassName('number-form')[0].addEventListener('submit', e => {
			e.preventDefault();
			this.changeBase.call(this, parseFloat(e.target[0].value));
		});
		document.getElementsByClassName('size-form')[0].addEventListener('submit', e => {
			e.preventDefault();
			this.changeSize.call(this, parseInt(e.target[0].value));
		});
