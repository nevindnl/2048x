		...
		_createClass(Game, [{
			...
			value: function start() {
				...
				document.getElementsByTagName('form')[0].addEventListener('submit', function (e) {
					e.preventDefault();
					var base = e.target[1].value;
					var size = e.target[2].value;
	
					if (base !== '') {
						_this.changeBase.call(_this, parseFloat(base));
					}
					if (size !== '') {
						_this.changeSize.call(_this, parseFloat(size));
					}
				});
			...
			value: function drawGrid() {
				...
				for (var i = 0; i < Math.pow(this.size, 2); i++) {
					...
					div.style.border = this.cellBorder + 'px solid #48d3d6';
		...
		_createClass(Board, [{
			...
			value: function adjCells(i, j) {
				...
				[-1, 1].forEach(function (d) {
					cells.push(_this.get(i + d, j));
					cells.push(_this.get(i, j + d));
				});
			...
			value: function leftTilt() {
				...
				this.grid = this.grid.map(function (row) {
					//remove spaces
					var tilted = row.filter(function (cell) {
						return cell.n !== -1;
					});
					...
					while (j < tilted.length - 1) {
						var _tilted$slice = tilted.slice(j, j + 2);
	
						var _tilted$slice2 = _slicedToArray(_tilted$slice, 2);
	
						var cell = _tilted$slice2[0];
						var nextCell = _tilted$slice2[1];
					...
					while (tilted.length < _this2.size) {
						tilted.push(new _cell2.default(-1));
					}
			...
			value: function over() {
				...
				return this.full() && !this.someCell(function (cell, i, j) {
					return _this3.adjCells(i, j).some(function (otherCell) {
	...
	var Cell = function () {
		...
		_createClass(Cell, [{
			...
			value: function draw(ctx, cellDim, base) {
				...
				var number = JSON.stringify(Math.round(Math.pow(base, this.n) * 100) / 100);
				var dec = number.length - 4;
