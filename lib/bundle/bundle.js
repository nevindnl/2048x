	...
	var Game = function () {
		...
		_createClass(Game, [{
			...
			value: function start() {
				...
				document.getElementsByClassName('number-form')[0].addEventListener('submit', function (e) {
					e.preventDefault();
					_this.changeBase.call(_this, parseFloat(e.target[0].value));
				});
				document.getElementsByClassName('size-form')[0].addEventListener('submit', function (e) {
					e.preventDefault();
					_this.changeSize.call(_this, parseInt(e.target[0].value));
				});
			...
			value: function drawGrid() {
				document.querySelectorAll('.grid-container div').forEach(function (div) {
					div.remove();
				});
				...
				var cellDim = this.cellDim - 2 * this.cellBorder + 'px';
				for (var i = 0; i < Math.pow(this.size, 2); i++) {
					var div = document.createElement('div');
					div.style.width = cellDim;
					div.style.height = cellDim;
					div.style.border = this.cellBorder + 'px solid #4B72CC';
			...
			value: function installHandlers() {
				var game = this;
				var keys = ['ArrowUp', 'w', 'j', 'ArrowDown', 's', 'k', 'ArrowLeft', 'a', 'h', 'ArrowRight', 'd', 'l'];
			...
			value: function end(type) {
				...
				var captionText = type === 'won' ? 'You win.' : 'Game over.';
				var buttonText = type === 'won' ? 'Play again' : 'Try again';
			...
			key: 'changeBase',
			value: function changeBase(base) {
				this.base = base;
		...
		_createClass(Board, [{
			...
			value: function leftTilt() {
				...
				this.grid = this.grid.map(function (row) {
					...
					//merge
					var j = 0;
					while (j < newRow.length - 1) {
						var _newRow$slice = newRow.slice(j, j + 2);
	
						var _newRow$slice2 = _slicedToArray(_newRow$slice, 2);
	
						var cell = _newRow$slice2[0];
						var nextCell = _newRow$slice2[1];
					...
					//add back spaces
					while (newRow.length < _this.size) {
						newRow.push(new _cell2.default(-1));
					}
			...
			key: 'full',
			value: function full() {
				return !this.someCell(function (cell) {
					return cell.n === -1;
				});
			}
		}, {
			...
			value: function over() {
				...
				return this.full() && !this.someCell(function (cell, i, j) {
					return _this2.adjCells(i, j).some(function (otherCell) {
						return cell.n === otherCell.n;
					});
				});
			}
		}, {
			...
			key: 'adjCells',
			value: function adjCells(i, j) {
				var _this3 = this;
	
				var cells = [];
				[-1, 1].forEach(function (d) {
					cells.push(_this3.get(i + d, j));
					cells.push(_this3.get(i, j + d));
				});
				return cells.filter(function (cell) {
					return cell !== undefined && cell.n !== -1;
				});
			}
		}]);
